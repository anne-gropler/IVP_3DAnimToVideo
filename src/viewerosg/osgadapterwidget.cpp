...
namespace VIEWEROSG
{
        ...
        public:
            ...
            {
                return mKeyMap.value(event->key(),int(*(event->text().toLocal8Bit().data())));
            }

        private:
            typedef QMap<unsigned int, int> KeyMap;
            KeyMap mKeyMap;
        };
        ...
        void OSGAdapterWidget::threadedInitializeGL()
        {
            ...
            mThreadGLContext = new QGLContext(mGLFormat, this);
            if (!mThreadGLContext->create(context()))
            {
                osg::notify(osg::WARN) << "Failed to create shared context for draw thread" << std::endl;
            }

            // don't change this widgets context, but let the OSG draw thread use mThreadGLContext (via threadedMakeCurrent()) 
            // because setContext() causes the widget to go black permanently (has to do with the use of setParent() in setContext()
        ...
        //////////////////////////////////////////////////////////////////////////////////
        void OSGAdapterWidget::threadedDoneCurrent() 
        {
            if (mThreadGLContext)
                mThreadGLContext->doneCurrent();
            else
                doneCurrent();
        }
        ...
        {
            //TODO!: need to set scrollingdelta??
            if (mGraphicsWindow.valid())
            {
